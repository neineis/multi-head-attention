...
class Beam(object):
    ...
    def advance(self, wordLk, copyLk, attnOut,mulAttnOut):
        ...
        self.mul_head_attn.append(mulAttnOut.index_select(0,prevK))
    ...
    def getHyp(self, k):
        ...
        # print('len(self.prevKs), len(self.nextYs), len(self.attn):',len(self.prevKs), len(self.nextYs), len(self.attn),len(self.mul_head_attn))
        for j in range(len(self.prevKs) - 1, -1, -1):
            hyp.append(self.nextYs[j + 1][k])
            attn.append(self.attn[j][k])
            mulattn.append(self.mul_head_attn[j][k])
            isCopy.append(self.isCopy[j][k])
            copyPos.append(self.nextYs_true[j + 1][k])
            k = self.prevKs[j][k]

        return hyp[::-1],  isCopy[::-1], copyPos[::-1], torch.stack(attn[::-1]), torch.stack(mulattn[::-1])
